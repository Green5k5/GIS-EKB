...
window.markersBySoslovie = {};
window.markersIndex = {};

function getSoslovieColor(name) {
  const palette = {
    'Дворяне':'#e74c3c',
    'Купец':'#3498db',
    'Крестьяне':'#2ecc71',
...
function openScan(item) {
  ...
  if (!modal || !img) return;
  img.src = item.scan || 'https://placehold.co/1200x800?text=Нет+скана';
...
function renderMap(filtered) {
  ...
  filtered.forEach(item => {
    ...
    const color = getSoslovieColor(item.soslovie || item.settlement);
    let m;
    ...
    h += `</div>`;
    m.bindPopup(h + `<div style="margin-top:10px"><button onclick="openScan(${JSON.stringify('x')})">Открыть скан</button></div>`, { className: "custom-popup", maxWidth: 300 });
    window.markersIndex[item.num] = m;
    if (!window.markersBySoslovie[item.soslovie]) window.markersBySoslovie[item.soslovie] = [];
    window.markersBySoslovie[item.soslovie].push(m);
